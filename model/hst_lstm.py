...
class STLSTM(nn.Module):
    ...
    def check_forward_input(self, input_l, input_s, input_q):
        if not (input_l.size(1) == input_s.size(1) == input_q.size(1)):
            raise RuntimeError(
                "input has inconsistent input_size: got {}, expected {}".format(
                    input_l.size(1), self.input_size))
...
class HSTLSTM_CLS(nn.Module):
    def __init__(self, hidden_size, item_num, tslot_num, sslot_num):
        super(HSTLSTM_CLS, self).__init__()
        self.item_num = item_num
        self.hidden_size = hidden_size
        # Temporal slot and spatial slot are designed for span/differentiation value. 
        self.temporal_slots = tslot_num
        self.spatial_slots = sslot_num
        self.item_emb = nn.Parameter(torch.Tensor(item_num, hidden_size))
        self.t_emb = nn.Parameter(torch.Tensor(tslot_num+1, hidden_size))
        self.s_emb = nn.Parameter(torch.Tensor(sslot_num+1, hidden_size))
        self.rnn = STLSTM(hidden_size, hidden_size)
    ...
    def forward(self, seq_idx, t_idx, t_idx_2, s_idx, s_idx_2, coef_t, coef_s, state=None):
        x = self.item_emb[seq_idx]
        coef_t, coef_s = coef_t.unsqueeze(-1), coef_s.unsqueeze(-1)
        t_repr = self.t_emb[t_idx] * coef_t + self.t_emb[t_idx_2] * (1-coef_t)
        s_repr = self.s_emb[s_idx] * coef_s + self.s_emb[s_idx_2] * (1-coef_s)
        out, cell = self.rnn(x, s_repr, t_repr, state)
